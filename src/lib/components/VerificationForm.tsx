...
export default function VerificationForm({ userId, email }: { userId: string, email: string }) {
  const [state, formAction] = useFormState(verifyAccount, initialState);

  console.log(state.message)
  return (
    <div className="space-y-4"><p>Please verify your account</p>
      ...
      <form action={resendVerificationCode}>
        ...
        <input name="userId" value={userId} hidden readOnly />
        <button type="submit" className='bg-white text-black' >Resend Verification Code</button>
      </form>
    </div>
  )
}
